        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Thêm thông tin và tải lên file đề thi của bạn.
            </p>
          </div>
        </div>
                              ...
                              <tr key={q.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3 text-gray-500">
                                  <span className="flex items-center gap-1">
                                    <Tag className="h-3 w-3" /> {q.subject} -{" "}
                                    {q.status}
                                  </span>
                                </td>
                                <td className="px-4 py-3 text-center">
                                  <Badge
                                    className={cn(
                                      "font-normal border shadow-none pointer-events-none",
                                      getLevelColor(q.level),
                                    )}
                                  >
                                    {q.level}
                                  </Badge>
            ...
            {selectedQuestionIds.length > 0 ? (
              <div className="border border-blue-100 rounded-lg bg-blue-50/50 p-4">
                <p className="text-sm font-medium text-blue-800 mb-3">
                  Đã thêm {selectedQuestionIds.length} câu hỏi từ Ngân hàng
                </p>
                <ul className="space-y-2 max-h-64 overflow-y-auto pr-2">
                  {selectedQuestionsDetails.map((q, idx) => (
                    <li
                      key={q.id}
                      className="bg-white p-3 rounded border border-gray-200 shadow-sm flex items-start gap-3"
                    >
                      <span className="bg-blue-100 text-blue-700 text-xs font-bold w-6 h-6 rounded flex items-center justify-center shrink-0">
                        {idx + 1}
                      </span>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              {Number(examData.targetQuestions) > 0 &&
                selectedQuestionIds.length > 0 &&
                selectedQuestionIds.length <
                  Number(examData.targetQuestions) && (
              ...
              {Number(examData.targetQuestions) > 0 &&
                selectedQuestionIds.length >=
                  ...
                  <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-700 text-xs">
                    <span className="font-semibold">&#10003;</span>
                    Đã đủ {selectedQuestionIds.length}/
                    {examData.targetQuestions} câu hỏi
                  </div>
                )}
            </div>
          </div>
